...
| §33 | Empty / loading / error / offline 상태 정비 | 오늘·학교·할 일·검색은 처리됨. 캘린더 하위 뷰와 공유·스포츠 화면이 남음 |
...
- **서버 마이그레이션이 필요하다.** `supabase/migrations/20260810_events_payload.sql` 을
  적용해야 클라우드 동기화가 무손실이 된다. 적용 전에는 앱이 자동으로 레거시
  형식으로 내려가므로 동작은 하지만, 시각·반복 정보는 여전히 왕복에서 사라진다.
